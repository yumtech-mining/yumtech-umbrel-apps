...
function renderMiners() {
  ...
  table.innerHTML = rows.map((miner) => {
    ...
    const agent = miner.user_agent ? `<small title="${escapeHtml(miner.user_agent)}">${escapeHtml(miner.user_agent)}</small>` : `<small>${escapeHtml(shortHash(miner.btc_address, 12, 8))}</small>`;
    return `<tr>
...
function filteredShares() {
  ...
  return state.shares.filter((share) => [share.worker_name, share.btc_address, share.job_id, share.share_hash]
    .some((value) => String(value || "").toLowerCase().includes(query)));
}
...
function renderShares() {
  ...
  table.innerHTML = rows.map((share) => {
    ...
    return `<tr>
      ...
      <td class="mono">${escapeHtml(shortHash(share.job_id, 8, 6))}</td>
...
function renderConfig() {
  ...
  text("config-sv2", `TCP :${config.sv2_port || 3340}`);
  text("config-vardiff", `${formatDifficulty(config.vardiff_min)} — ${formatDifficulty(config.vardiff_max)}`);
  text("config-share-target", `Dakikada ${formatNumber(config.target_shares_per_minute || 12)} hedef share`);
  text("config-coinbase", config.coinbase_signature || "/YUMTECH/");
  text("config-version-mask", config.version_rolling_mask || "1fffe000");
  text("config-engine", config.engine || "YUMTECH Engine");
  text("config-engine-build", config.engine_build || "Build bilgisi yok");
  text("hero-protocol", `SV1 :${config.sv1_port || 3333} + SV2 :${config.sv2_port || 3340}`);
  ...
  if (keyElement) {
    keyElement.textContent = publicKey || "Public key hazırlanıyor…";
